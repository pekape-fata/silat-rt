...
-- ---------------------------------------------------------
create table public.wilayah_rw (
...
alter table public.wilayah_rt
    add column wilayah_rw_id uuid references public.wilayah_rw(id) on delete restrict;
...
alter table public.pengurus_rt
    add column wilayah_rw_id uuid references public.wilayah_rw(id) on delete cascade,
...
-- ---------------------------------------------------------
create table public.pelimpahan_wewenang (
    id uuid primary key default gen_random_uuid(),
...
create index idx_pelimpahan_pemberi on public.pelimpahan_wewenang(pemberi_wewenang_id, status);
...
alter table public.surat
    add column ditandatangani_rt_oleh uuid references public.users(id) on delete set null,
    add column ditandatangani_rw_oleh uuid references public.users(id) on delete set null,
    add column atas_nama_pelimpahan_rt boolean not null default false,
    add column atas_nama_pelimpahan_rw boolean not null default false,
    add column catatan_perbaikan text,
    add column no_hp_pemohon varchar(20),   -- dipakai saat pengajuan publik, sebelum warga terhubung akun
    add column diajukan_tanpa_login boolean not null default false,
...
alter table public.surat
    add column nik_pemohon varchar(16),
    add column nama_pemohon varchar(150);
...
-- ---------------------------------------------------------
create table public.notifikasi_surat (
    id uuid primary key default gen_random_uuid(),
